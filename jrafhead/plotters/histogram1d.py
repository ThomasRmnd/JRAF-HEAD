...
from jrafhead.loader import MCChengzhuoTemplateHistogram
from jrafhead.utils import Histogram, uniform_bins
...
class Histogram1DPlotter(BasePlotter):
    ...
    density : bool
        If True, normalize each histogram such that its integral is 1.
        For non-uniform bins, the normalization is performed using the bin
    widths.
    **kwargs
    ...
    def add_histogram(
        ...
        if self.density:
            norm  = np.sum(hist)
            err_  = err  / (norm * self.widths)
            hist_ = hist / (norm * self.widths)
        else:
            err_  = err
            hist_ = hist
...
class PromptEnergyPlotter(Histogram1DPlotter):
    ...
    **kwargs
    ...
    def __init__(self, binmode: str = "nmo", **kwargs: Any) -> None:
        ...
        kwargs.setdefault("xlim",   (0, 12.5))
        kwargs.setdefault("xlabel", r"$E_{p}$ (MeV)")
    ...
    def __init__(
        ...
        **kwargs:       Any
        ...
        if binmode == "nmo":
            ...
            bins = PROMPT_ENERGY_BINS_UNIFORM
        kwargs.setdefault("bins",   bins)
        kwargs.setdefault("xlim",   (bins[0], bins[-1]))
        kwargs.setdefault("ylim",   (0.0, 50.0))
        kwargs.setdefault("xlabel", r"$E_{p}$ (MeV)")
        kwargs.setdefault("ylabel", r"Relative uncertainty (\%)")
        kwargs.setdefault("show_errors", False)
        kwargs.setdefault("show_steps", True)
        kwargs.setdefault("enable_fit", False)
        super().__init__(**kwargs)
...
class Li9He8ShapeGroupCFitPlotter(PromptEnergyPlotter):
    ...
    def __init__(self, template: Histogram, **kwargs):
        super().__init__(**kwargs)
    ...
    def plot(self) -> tuple[plt.Figure, dict[str, plt.Axes]]:
        ...
        if any(ds["label"] for ds in self.datasets) or self.fit_result is not None:
    ...
    def _maybe_fit(self, ax) -> None:
        ...
        self._y_fit = fitter.model_full(*self.fit_result.popt)
...
class Li9He8ChengzhuoFitSmearingPlotter(PromptEnergyPlotter):
    ...
    def _maybe_fit(self, ax) -> None:
        ...
        histrb0    *= 1.0 / np.sum(histrb0 * self.widths)
        # print(f"[Integral0]: Old = {integr0}, New = {np.sum(histrb0 * np.diff(self.bins))}")
        ...
        histrb1    *= 1.0 / np.sum(histrb1 * self.widths)
        # print(f"[Integral1]: Old = {integr1}, New = {np.sum(histrb1 * np.diff(self.bins))}")
        ...
        histrb2    *= 1.0 / np.sum(histrb2 * self.widths)
        # print(f"[Integral2]: Old = {integr2}, New = {np.sum(histrb2 * np.diff(self.bins))}")
        ...
        histrb3    *= 1.0 / np.sum(histrb3 * self.widths)
        # print(f"[Integral3]: Old = {integr3}, New = {np.sum(histrb3 * np.diff(self.bins))}")
        ...
        histrb4    *= 1.0 / np.sum(histrb4 * self.widths)
        # print(f"[Integral4]: Old = {integr4}, New = {np.sum(histrb4 * np.diff(self.bins))}")
        ...
        self._y_fit = fitter.model_full(self.centers, *self.fit_result.popt)
    ...
    def _draw_fit_text(self, ax) -> None:
        ...
        N,  f0,  f1,  f2,  f3  = self.fit_result.popt
        Ne, f0e, f1e, f2e, f3e = self.fit_result.perr
...
class DelayedEnergyPlotter(Histogram1DPlotter):
    ...
    def _maybe_fit(self, ax: plt.Axes) -> None:
        ...
        from jrafhead.fits.function.functions import GaussianFitter
...
class PromptDelayedTimePlotter(Histogram1DPlotter):
    ...
    def __init__(
        ...
        **kwargs:       Any
        ...
        kwargs.setdefault("bins",   PROMPT_DELAYED_DT_BINS)
        kwargs.setdefault("xlim",   (0, 1.02))
        kwargs.setdefault("xlabel", r"$\Delta t_{p-d}$ (ms)")
        kwargs.setdefault("ylabel", r"Entries")
        super().__init__(**kwargs)
...
class PromptDelayedDistancePlotter(Histogram1DPlotter):
    """
    Prompt–delayed vertex distance d_{p-d} in metres.
    """
    ...
    def __init__(
        ...
        **kwargs:       Any
        ...
        kwargs.setdefault("bins",   PROMPT_DELAYED_DR_BINS)
        kwargs.setdefault("xlim",   (0, 1.55))
        kwargs.setdefault("xlabel", r"$d_{p-d}$ (m)")
        kwargs.setdefault("ylabel", r"Entries")
        super().__init__(**kwargs)
        self.accidental_fit = accidental_fit
    ...
    def _maybe_fit(self, ax: plt.Axes) -> None:
        ...
        from fits import CubicFitter

        hist = self.datasets[0]["hist"]
        err  = self.datasets[0]["err"]

        if not self.accidental_fit:
            return

        fitter = CubicFitter(self.bins, hist, err)
        self.fit_result = fitter.fit()
        if self.fit_result is None:
            return

        x_smooth = uniform_bins(self.bins[0], self.bins[-1], 500)
        y_smooth = fitter.model(x_smooth, *self.fit_result.popt)
    ...
    def _get_fit_info_text(self):
        A     = self.fit_result.popt[0]
        A_err = self.fit_result.perr[0]
        text = (
            r"$P(\chi^2/\mathrm{ndf} = %.1f / %d) = %.3f$" "\n\n"
            r"$A = %.1f \pm %.1f$"
        ) % (
...
class Li9He8RateEstimationPlotter(Histogram1DPlotter):
    ...
    def _maybe_fit(self, ax: plt.Axes) -> None:
        ...
        N9li8he, f9li, Nbkg, Rmu = self.fit_result.popt
        ...
        # ytot_smooth    = fitter_smooth._predict(N9li8he, f9li, t9li, t8he, Nbkg, Rmu)
        # y9li_smooth    = fitter_smooth.model_9li(x_smooth, N9li8he, f9li, t9li, Rmu)
        # y8he_smooth    = fitter_smooth.model_8he(x_smooth, N9li8he, f9li, t8he, Rmu)
        # ybkg_smooth    = fitter_smooth.model_bkg(x_smooth, Nbkg, Rmu)
        ytot_smooth    = fitter_smooth._predict(N9li8he, f9li, Nbkg, Rmu)
        y9li_smooth    = fitter_smooth.model_9li(x_smooth, N9li8he, f9li, t9li, Rmu)
        y8he_smooth    = fitter_smooth.model_8he(x_smooth, N9li8he, f9li, t8he, Rmu)
        ybkg_smooth    = fitter_smooth.model_bkg(x_smooth, Nbkg, Rmu)
        # ytot_smooth    = fitter_smooth._predict(N9li8he, Nbkg, Rmu)
        # y9li8he_smooth = fitter_smooth.model_9li8he(x_smooth, N9li8he, t9li8he, Rmu)
        # ybkg_smooth    = fitter_smooth.model_bkg(x_smooth, Nbkg, Rmu)

        ax.plot(
            x_smooth, ytot_smooth,
            linestyle="--", linewidth=1.8, color=CUSTOM_MARKEDRED, zorder=4, label=r"Fit"
        )
        ...
        #     linestyle="--", linewidth=1.8, color=CUSTOM_BLUE, zorder=4, label=r"$^{9}\mathrm{Li}/^{8}\mathrm{He}$"
        # )

        ax.plot(
            x_smooth, ybkg_smooth,
            linestyle="--", linewidth=1.8, color=CUSTOM_ORANGE, zorder=4, label=r"Background"
        )
    ...
    def _get_fit_info_text(self):
        ...
        # )
        N9li8he,    f9li,    Nbkg,    Rmu    = self.fit_result.popt
        N9li8heerr, f9lierr, Nbkgerr, Rmuerr = self.fit_result.perr
        text = (
            r"$P(\chi^{2} / \mathrm{ndf} = %.1f / %d)  = %.3f$"      "\n\n"
            r"$N_{^{9}\mathrm{Li}/^{8}\mathrm{He}} = %.1f \pm %.1f$" "\n"
            r"$f_{^{9}Li} = %.3f \pm %.3f$"                          "\n"
...
class Li9RateEstimationPlotter(Histogram1DPlotter):
    """
    9Li rate estimation via time to last muon with a spallation neutron distribution.
    ...
    def _maybe_fit(self, ax: plt.Axes) -> None:
        ...
        from fits import Li9RateEstimationFitter
        ...
        fitter = Li9RateEstimationFitter(self.bins, hist, err, xlim=(t_min_fit, self.bins[-1]))
        self.fit_result = fitter.fit()
        if self.fit_result is None:
            return
        # from fits import FitResult
        ...
        from config import CUSTOM_BLUE, CUSTOM_MARKEDRED, CUSTOM_ORANGE

        N9li, Nbkg, Rmu = self.fit_result.popt
        t9li = 0.257

        bins_smooth   = np.linspace(t_min_fit, self.bins[-1], 501)
        x_smooth      = 0.5 * (bins_smooth[1:] + bins_smooth[:-1])
        fitter_smooth = Li9RateEstimationFitter(bins_smooth, np.zeros_like(x_smooth), np.ones_like(x_smooth))
        ytot_smooth   = fitter_smooth._predict(N9li, Nbkg, Rmu)
        y9li_smooth   = fitter_smooth.model_9li(x_smooth, N9li, t9li, Rmu)
        ybkg_smooth   = fitter_smooth.model_bkg(x_smooth, Nbkg, Rmu)
    ...
    def _get_fit_info_text(self):
        N9li,    Nbkg,    Rmu    = self.fit_result.popt
        N9lierr, Nbkgerr, Rmuerr = self.fit_result.perr
        text = (
            r"$P(\chi^{2} / \mathrm{ndf} = %.1f / %d)  = %.3f$"      "\n"
            r"$N_{^{9}\mathrm{Li}} = %.1f \pm %.1f$"                 "\n"
            r"$N_{\mathrm{bkg}} = %.1f \pm %.1f$"                    "\n"
            r"$R_{\mu} =  %.2f \pm %.2f~\mathrm{cps}$"
        ) % (
            self.fit_result.chi2, self.fit_result.ndf, self.fit_result.pvalue,
...
class Li9B12RateEstimationPlotter(Histogram1DPlotter):
    """
    B12/9Li rate estimation via time to last muon with a spallation neutron distribution.
    ...
    def _maybe_fit(self, ax: plt.Axes) -> None:
        ...
        from fits import Li9B12RateEstimationFitter
        ...
        fitter = Li9B12RateEstimationFitter(self.bins, hist, err, xlim=(t_min_fit, self.bins[-1]))
        self.fit_result = fitter.fit()
        if self.fit_result is None:
            return
        # from fits import FitResult
        ...
        N12b, N9li, Nbkg, Rmu = self.fit_result.popt
        t12b = 0.02020 / np.log(2) # t_{1/2} = 20.20(2) ms
        t9li = 0.1782  / np.log(2) # t_{1/2} = 178.2(4) ms

        bins_smooth   = np.linspace(t_min_fit, self.bins[-1], 501)
        x_smooth      = 0.5 * (bins_smooth[1:] + bins_smooth[:-1])
        fitter_smooth = Li9B12RateEstimationFitter(bins_smooth, np.zeros_like(x_smooth), np.ones_like(x_smooth))
        ytot_smooth   = fitter_smooth._predict(N12b, N9li, Nbkg, Rmu)
        y12b_smooth   = fitter_smooth.model_12b(x_smooth, N12b, t12b, Rmu)
        y9li_smooth   = fitter_smooth.model_9li(x_smooth, N9li, t9li, Rmu)
        ybkg_smooth   = fitter_smooth.model_bkg(x_smooth, Nbkg, Rmu)
        ...
        ax.plot(
            x_smooth, y12b_smooth,
            linestyle="--", linewidth=1.8, color=CUSTOM_MARKEDGREEN, zorder=4,
        )
    ...
    def _get_fit_info_text(self):
        N12b,    N9li,    Nbkg,    Rmu    = self.fit_result.popt
        N12berr, N9lierr, Nbkgerr, Rmuerr = self.fit_result.perr
        text = (
            r"$P(\chi^{2} / \mathrm{ndf} = %.1f / %d)  = %.3f$"      "\n"
            r"$N_{^{12}\mathrm{B}} = %.1f \pm %.1f$"                 "\n"
            r"$N_{^{9}\mathrm{Li}} = %.1f \pm %.1f$"                 "\n"
            r"$N_{\mathrm{bkg}} = %.1f \pm %.1f$"                    "\n"
            r"$R_{\mu} =  %.2f \pm %.2f~\mathrm{cps}$"
        ) % (
...
class ShapeComparisonPlotter(Histogram1DPlotter):
    """
    Shape comparison plotter. Draws the individual curve and compare them to the
    first one, or the mean.
    ...
    def __init__(self, compare_mean: bool = True, **kwargs):
        super().__init__(**kwargs)
        self.compare_mean = compare_mean
    ...
    def plot(self) -> tuple[plt.Figure, dict[str, plt.Axes]]:
        ...
        for i, d in enumerate(self.datasets):
            self._draw_dataset(ax_main, d)
            if self.compare_mean or (not self.compare_mean and i != 0):
                self._draw_residuals(ax_resi, d)

        if self.compare_mean:
            hists = np.array([ds["hist"] for ds in self.datasets])
            refhist = np.mean(hists, axis=0)
            ax_main.step(
                self.bins,
                np.r_[refhist, refhist[-1]],
                where="post",
                color="gray",
                linewidth=1.8,
                linestyle="--",
    ...
    def _draw_residuals(self, ax: plt.Axes, d: _Dataset) -> None:
        hist, err = d["hist"], d["err"]
        if self.compare_mean:
            hists = np.array([ds["hist"] for ds in self.datasets])
            errs  = np.array([ds["err"]  for ds in self.datasets])

            refhist = np.mean(hists, axis=0)
            referr = np.sqrt(np.sum(errs**2, axis=0)) / len(self.datasets)
        else:
            refhist = self.datasets[0]["hist"]
            referr  = self.datasets[0]["err"]
        pull = np.full(err.shape, np.nan)
        mask = np.logical_and(err > 0, referr > 0)
        pull[mask] = (hist[mask] - refhist[mask]) / referr[mask]

        ax.errorbar(
            self.centers[mask],
            pull[mask],
            yerr=err[mask] / referr[mask],
            xerr=self.widths[mask] / 2,
            label=d["label"],
            fmt="o", color=d["linecolor"],
            markersize=4.5, zorder=3,
        )
    ...
    def _style_residual_axis(self, ax: plt.Axes) -> None:
        ...
        if self.xlim:
            ax.set_xlim(*self.xlim)
        ax.set_ylim(-7.0, 7.0)
        ax.axhline(0.0, color="gray", linewidth=1.5, linestyle="--", zorder=2)
        ax.axhspan(-1.0, 1.0, color="gray", alpha=0.25, zorder=1)
        ax.axhspan(-2.0, 2.0, color="gray", alpha=0.10, zorder=0)


class ShapeModelComparisonPlotter(Histogram1DPlotter):
    """
    Prompt energy plotter for the Li9/He8 shape analysis.  Draws the
    signal-background difference spectrum, fits a fixed-shape MC template
    to it with a single free amplitude N, and shows the fit residual (pull) 
    ...
    Layout (GridSpec 1 x n + 1), with n the number of model.
    ---------------------
    ┌──────────┐
    │  main    │
    ├──────────┤
    ...
    def __init__(
        ...
        kwargs.setdefault("ylabel", r"Entries")
        super().__init__(bins, xlim=xlim, xlabel=xlabel, **kwargs)
        nmodels = len(models)
        if not all(
            len(x) == nmodels
            for x in (
            ...
            print("Warning: All model configuration lists must have the same length")

        self.models: list[_Dataset] = []
        for model, linecolor, linestyle, fillcolor, alpha, label in zip(
            models,
            linecolors,
            linestyles,
            fillcolors,
    ...
    def plot(self) -> tuple[plt.Figure, dict[str, plt.Axes]]:
        ...
        fig = plt.figure(figsize=(7, 6 + 2 * nmodels))
        height_ratios = [4.0] + [1.2] * nmodels
        gs = GridSpec(nmodels + 1, 1, height_ratios=height_ratios, hspace=0.1)
        ...
        for m in self.models:
            self._draw_models(ax_main, m)
        for d in self.datasets:
            self._draw_dataset(ax_main, d)
            for ax, m in zip(residual_axes, self.models):
                self._draw_residuals(ax, d, m)
    
        self.apply_style(ax_main)
        ax_main.set_xlabel("")
        plt.setp(ax_main.get_xticklabels(), visible=False)
        ax_main.legend(loc="upper right")
        for i, (ax, m) in enumerate(zip(residual_axes, self.models)):
            self._style_residual_axis(ax)
            ax.set_ylabel(
                r"$\mathrm{Resi.}\;(\sigma)$" "\n"
                r"$\scriptstyle \mathrm{" + m["label"] + r"}$",
                ha="center",
                va="center",
                labelpad=18,
            )
            if i == len(residual_axes) - 1:
                ax.set_xlabel(self.xlabel)
            else:
                ax.set_xlabel("")
                ax.tick_params(
        ...
        fig.align_ylabels([ax_main] + residual_axes)
    
        return fig, {"main": ax_main, "resi": residual_axes}

    def _draw_models(self, ax, m: _Dataset) -> None:
        ax.step(
            self.bins, np.r_[m["hist"], m["hist"][-1]],
            where="post", color=m["linecolor"], linewidth=1.8, zorder=4,
            label=m["label"],
        )
        
    def _draw_residuals(self, ax: plt.Axes, d: _Dataset, m: _Dataset) -> None:
        hist, err = d["hist"], d["err"]
        pull = np.full(err.shape, np.nan)
        mask = (err > 0)
        pull[mask] = (hist[mask] - m["hist"][mask]) / err[mask]
        ax.plot(self.centers, pull, "o", color=d["linecolor"], markersize=4.5, zorder=3, label=d["label"])
        ax.axhline(0.0, color=m["linecolor"], linewidth=1.5, linestyle="--", zorder=2)
        ax.axhspan(-1.0, 1.0, color=m["linecolor"], alpha=0.25, zorder=1)
        ax.axhspan(-2.0, 2.0, color=m["linecolor"], alpha=0.10, zorder=0)
        # ax.text(0.95, 0.90, m["label"], transform=ax.transAxes, ha="right", va="top")
...
class MuonLengthPlotter(Histogram1DPlotter):
    """
    Muon length (m).
    """

    def __init__(self, **kwargs: Any) -> None:
        kwargs.setdefault("bins",   uniform_bins(0.0, 40.0, 1000))
        kwargs.setdefault("xlim",   (0.0, 40.0))
        kwargs.setdefault("xlabel", r"$L_{\mu}$ (m)")
        kwargs.setdefault("ylabel", r"Entries")
        super().__init__(**kwargs)
...
class MuonMultiplicityPlotter(Histogram1DPlotter):
    """
    Muon multiplicity.
    """

    def __init__(self, **kwargs: Any) -> None:
        kwargs.setdefault("bins",   uniform_bins(1.0, 11.0, 10))
        kwargs.setdefault("xlim",   (1.0, 11))
        kwargs.setdefault("ylim",   (0.5, None))
        kwargs.setdefault("yscale", "log")
        kwargs.setdefault("xlabel", r"$n_{\mathrm{track}}$")
        kwargs.setdefault("ylabel", r"Entries")
        super().__init__(**kwargs)
