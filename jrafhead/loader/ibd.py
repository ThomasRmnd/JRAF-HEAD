...
_IBD_BRANCHES = [
    ...
    "dt_last_mu_with_neu", "dt_last_mu", "dt_mu2p", "dlat_mu2p",
]
...
class IBDData:
    ...
    dt_last_mu:             np.ndarray  # (N,)      float   Time since last muon                (s)
    dt_mu2p:                np.ndarray  # (N,)      float   Time muon-to-prompt                 (s)
    dlat_mu2p:              np.ndarray  # (N,)      float   Distance muon-to-prompt             (mm)


def load_ibd(filepath: str, dirpath: str) -> IBDData:
    """
    Load and prepare all arrays for the IBD analysis.
    """
    file = uproot.open(filepath)
    raw  = file[f"{dirpath}/events"].arrays(_IBD_BRANCHES, library="np")
